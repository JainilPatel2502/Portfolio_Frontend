...
const LANGGRAPH_URL = "https://portfolio-api.jainilpatel.dev";
...
const Chatbot = () => {
  const [messages, setMessages] = useState([
    {
      type: "ai",
      content:
        "Hi! I'm a RAG-powered chatbot created by Jainil Patel. I'm here to help recruiters learn more about his work. Ask me anything!",
    },
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const stream = client.runs.stream(currentThreadId, ASSISTANT_ID, {
        input: inputData,
        streamMode: "messages",
      });
      ...
      setMessages((prev) => {
        ...
        return [
          ...newMessages,
          {
            type: "error",
            content:
              "Failed to connect to AI. Make sure the LangGraph server is running.",
          },
        ];
      });
    } finally {
    ...
    <>
        ...
        {isFocused && (
          ...
          >
            ...
            <div className="absolute top-6 left-6 flex items-center gap-3">
              ...
              <span className="text-[#a3e635] font-mono text-lg">
                Jainil's AI Assistant
              </span>
            </div>
            ...
            <div className="w-full max-w-3xl h-full overflow-y-auto px-4 space-y-4 no-scrollbar">
              {messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-[70%] p-4 rounded-2xl text-base leading-relaxed shadow-lg ${
                      msg.type === "user"
                  ...
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        a: ({ node, ...props }) => (
                          <a
                            className="text-[#a3e635] hover:underline font-medium"
                            target="_blank"
                        ...
                        code: ({
                          ...
                          return inline ? (
                            <code
                              className="bg-black/30 px-1 py-0.5 rounded text-sm font-mono text-[#a3e635]"
                              {...props}
                            >
                              {children}
                            </code>
                            ...
                            <code
                              className="block bg-black/30 p-3 rounded-lg text-sm font-mono text-gray-200 overflow-x-auto my-2 border border-white/10"
                              {...props}
                            >
                              {children}
                            </code>
      ...
      >
        <form onSubmit={handleSend} className="relative group">
          <div
            className={`absolute inset-0 bg-gradient-to-r from-[#a3e635]/20 to-blue-500/20 rounded-2xl blur-xl transition-opacity duration-500 ${isFocused ? "opacity-100" : "opacity-0 group-hover:opacity-50"}`}
          />
          <input
            type="text"
            value={input}
